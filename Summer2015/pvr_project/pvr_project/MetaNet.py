...
class MetaNet:
    ...
    def add_conv_layer(self, input, filter_shape, image_shape, poolsize):
        self.net.add_conv_layer(input, filter_shape, image_shape, poolsize)
        self.net.update_params()
    ...
    def get_input_dim(self):
        ...
        return set_x[0].shape
        # Old version using non-conv flattened input
        # return len(set_x[0])
    ...
    def train(self):
        ...
        for i in range(sys.maxsize):
            totalCost = 0
            # For each of the elements in the training set
            for j in range(len(train_set_x)):
                # x = np.array([train_set_x[j]]).transpose()
                # Add a new dimension to array to create (1,d,y,x) array
                x = np.expand_dims(train_set_x[j], axis=0).transpose(0,3,1,2)
                y = train_set_y[j]
                cost = (
